import type { LayoutRoute, Tab, TabStateSnapshot } from './layout-route';
...
export function createTabState(snapshot?: TabStateSnapshot) {
  ...
  const state = {
    ...
    reorderByKeys(keys: string[]) {
      ...
      const regular = keys.map(key => byKey.get(key)).filter((tab): tab is Tab => !!tab && !tab.affix);
    ...
    snapshot(): TabStateSnapshot {
      return { activeKey: state.activeKey, history: [...state.history], items: state.items.map(tab => ({ ...tab })) };
    },
    unpin(key: string) {
      const tab = state.items.find(item => item.key === key);
      if (!tab)
