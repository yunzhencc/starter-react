...
import Icon, { SunOutlined, SyncOutlined } from '@ant-design/icons';
...
export function ThemeToggle() {
  ...
  function setThemeWithTransition(event: KeyboardEvent<HTMLElement> | MouseEvent<HTMLElement>, nextTheme: ThemeMode) {
    if (theme === nextTheme) {
      return;
    }
    ...
    if (nextTheme === 'system' || !startViewTransition || !('clientX' in event) || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      update();
      return;
    }
    ...
    root.style.setProperty('color-scheme', nextResolvedTheme === 'dark' ? 'light' : 'dark', 'important');
    const transition = callViewTransition(document, startViewTransition, update);
    void transition.ready.then(() => {
      const clipPath = [`circle(0px at ${x}px ${y}px)`, `circle(${radius}px at ${x}px ${y}px)`];
      const animationConfig = getThemeTransitionAnimation(isDark, clipPath);
      root.style.setProperty('color-scheme', colorScheme, colorSchemePriority);
      const animation = root.animate(
        { clipPath: animationConfig.clipPath },
        {
          duration: 500,
          easing: 'ease-in',
          pseudoElement: animationConfig.pseudoElement,
        },
      );
  ...
  const menu: MenuProps = {
    items: themeMenuModes.map(mode => ({
      extra: theme === mode ? badge : null,
      icon: mode === 'system' ? <SyncOutlined /> : mode === 'light' ? <SunOutlined /> : <DarkTheme />,
      key: mode,
      label: mode === 'system' ? '跟随系统' : mode === 'light' ? '浅色主题' : '暗黑主题',
    })),
    onClick: ({ domEvent, key }) => setThemeWithTransition(domEvent, key as ThemeMode),
  };
