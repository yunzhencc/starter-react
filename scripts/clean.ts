...
async function processItem(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      ...
      console.error(
        `❌ Error handling item ${item} in ${currentDir}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    return false;
  }
...
async function cleanTargetsRecursively(
  ...
  try {
    ...
    console.warn(`Cannot read directory ${currentDir}: ${error instanceof Error ? error.message : String(error)}`);
    return;
  }
